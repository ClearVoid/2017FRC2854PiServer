...
public class ImageUtil {
	...
	public static BufferedImage colorCut(BufferedImage img, float[] averages,
			float[] standardDevations, float Standarderror) {
		...
		for (int x = 0; x < img.getWidth(); x++) {
			for (int y = 0; y < img.getHeight(); y++) {
				...
				if (Math.abs(zR) <= Standarderror
						&& Math.abs(zB) <= Standarderror
						&& Math.abs(zG) <= Standarderror) {
					out.setRGB(x, y, rgb);
				} else {
	...
	public static int[][] grayScale(BufferedImage img, int type) {
		...
		for (int x = 0; x < img.getWidth(); x++) {
			for (int y = 0; y < img.getHeight(); y++) {
				...
				// System.out.println("x: " + x + " imgWidth: " + img.getWidth()
				// + " imgHeight: " + img.getHeight() + " arrayWidth: " +
				// out[0].length + " arrayHeight: " + out.length);
				out[y][x] = (int) avg;
			}
		}
		return out;
	...
	public static double getDistance(int screenWidth, int projectedWidth,
			double actualWidth, double tanAngle) {
